import { applyProductsLocale, applyProductLocale } from './product-locale';
...
let cachedStoreLanguages = null;

export async function getStoreLanguages() {
  if (cachedStoreLanguages) return cachedStoreLanguages;

  const query = `
    query {
      localization {
        availableLanguages { isoCode }
      }
    }
  `;

  const data = await shopifyFetch({ query });
  cachedStoreLanguages =
    data?.localization?.availableLanguages?.map((lang) => lang.isoCode) || ['EN'];
  return cachedStoreLanguages;
}

function shouldUseArabicFallback(locale) {
  if (locale !== 'ar') return false;
  if (!cachedStoreLanguages) return true;
  return !cachedStoreLanguages.includes('AR');
}
...
export function resolveVariantFromProduct(product, color = "", size = "") {
  ...
  const normalizedColor = normalizeVariantOptionValue(color);
  const normalizedSize = normalizeVariantOptionValue(size);
  ...
  const variant = product.variants.find((v) => {
    ...
    const sizeMatch = sizeOpt
      ? normalizeVariantOptionValue(sizeOpt.value) === normalizedSize
      : true;
    const colorMatch = colorOpt
      ? normalizeVariantOptionValue(colorOpt.value) === normalizedColor
      : true;
...
export async function getProducts(locale = 'en') {
  if (productsCache.has(locale)) return productsCache.get(locale);

  const promise = fetchProductsUncached(locale);
  productsCache.set(locale, promise);
  return promise;
}

async function fetchProductsUncached(locale = 'en') {
  await getStoreLanguages();

  const query = `
    query getProducts($first: Int!, $after: String, $language: LanguageCode) @inContext(country: EG, language: $language) {
  ...
  while (hasNextPage) {
    ...
    if (!data?.products?.edges?.length) {
      if (allProducts.length === 0) {
        const mock = MOCK_PRODUCTS.map(populateMockVariants);
        return shouldUseArabicFallback(locale) ? applyProductsLocale(mock, locale) : mock;
      }
      break;
    }

    allProducts.push(...data.products.edges.map((e) => formatShopifyProduct(e.node)));
    hasNextPage = data.products.pageInfo?.hasNextPage ?? false;
    cursor = data.products.pageInfo?.endCursor ?? null;
  }

  return shouldUseArabicFallback(locale) ? applyProductsLocale(allProducts, locale) : allProducts;
}

export async function getProduct(handle, locale = 'en') {
  const cacheKey = `${locale}:${handle}`;
  if (productCache.has(cacheKey)) return productCache.get(cacheKey);

  const promise = fetchProductUncached(handle, locale);
  productCache.set(cacheKey, promise);
  return promise;
}

async function fetchProductUncached(handle, locale = 'en') {
  await getStoreLanguages();

  const query = `
    query getProduct($handle: String!, $language: LanguageCode) @inContext(country: EG, language: $language) {
  ...
  if (!data || !data.product) {
    ...
    const product = mock ? populateMockVariants(mock) : null;
    return product && shouldUseArabicFallback(locale)
      ? applyProductLocale(product, locale)
      : product;
  }

  const product = formatShopifyProduct(data.product);
  return shouldUseArabicFallback(locale) ? applyProductLocale(product, locale) : product;
}
...
export async function loginCustomer(email, password) {
  ...
  const data = await shopifyFetch({ query, variables, strict: true });
