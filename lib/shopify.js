...
async function fetchProductsUncached(locale = 'en') {
  ...
  const query = `
    query getProducts($language: LanguageCode) @inContext(country: EG, language: $language) {
      products(first: 24) {
        edges {
          node {
            id
  ...
  const products = data.products.edges.map(e => formatShopifyProduct(e.node));
  return shouldUseArabicFallback(locale) ? applyProductsLocale(products, locale) : products;
}
