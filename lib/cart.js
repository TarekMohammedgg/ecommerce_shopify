...
import { resolveVariantFromProduct } from './shopify-variants';
...
export function CartProvider({ children }) {
  ...
  const addToCart = (product, quantity = 1, color = "", size = "") => {
    if (!isLoggedIn) {
      const hasStoredToken = typeof window !== 'undefined' && !!localStorage.getItem(AUTH_TOKEN_KEY);
      if (authLoading && hasStoredToken) return;
